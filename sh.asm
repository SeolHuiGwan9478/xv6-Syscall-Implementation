...
main(void)
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
      ...
      27:	e8 4e 0d 00 00       	call   d7a <open>
...
fork1(void)
{
  ...
  if(pid == -1)
    ...
    panic("fork");
      ...
      96:	50                   	push   %eax
      97:	e8 0e 0d 00 00       	call   daa <close>
      break;
  ...
  exit();
      ...
      if(chdir(buf+3) < 0)
      d6:	e8 6f 0c 00 00       	call   d4a <chdir>
...
00000170 <runcmd>:
{
  ...
  switch(cmd->type){
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     19c:	52                   	push   %edx
     19d:	50                   	push   %eax
     19e:	e8 97 0b 00 00       	call   d3a <exec>
  ...
  if(pid == -1)
    ...
    close(rcmd->fd);
     ...
     1e0:	e8 c5 0b 00 00       	call   daa <close>
    if(open(rcmd->file, rcmd->mode) < 0){
     1e5:	59                   	pop    %ecx
     1e6:	58                   	pop    %eax
     1e7:	ff 73 10             	pushl  0x10(%ebx)
     1ea:	ff 73 08             	pushl  0x8(%ebx)
     1ed:	e8 88 0b 00 00       	call   d7a <open>
  ...
  if(pid == -1)
    ...
    close(p[0]);
     ...
     24b:	e8 5a 0b 00 00       	call   daa <close>
    close(p[1]);
     250:	58                   	pop    %eax
     251:	ff 75 f4             	pushl  -0xc(%ebp)
     254:	e8 51 0b 00 00       	call   daa <close>
    wait();
  ...
  if(pid == -1)
    ...
    panic("fork");
     ...
     2aa:	6a 00                	push   $0x0
     2ac:	e8 f9 0a 00 00       	call   daa <close>
      dup(p[0]);
     2b1:	5a                   	pop    %edx
     2b2:	ff 75 f0             	pushl  -0x10(%ebp)
     2b5:	e8 98 0a 00 00       	call   d52 <dup>
      close(p[0]);
     2ba:	59                   	pop    %ecx
     2bb:	ff 75 f0             	pushl  -0x10(%ebp)
     2be:	e8 e7 0a 00 00       	call   daa <close>
      close(p[1]);
     2c3:	58                   	pop    %eax
     2c4:	ff 75 f4             	pushl  -0xc(%ebp)
     2c7:	e8 de 0a 00 00       	call   daa <close>
      runcmd(pcmd->right);
     2cc:	58                   	pop    %eax
     ...
     2e5:	6a 01                	push   $0x1
     2e7:	e8 be 0a 00 00       	call   daa <close>
      dup(p[1]);
     2ec:	58                   	pop    %eax
     2ed:	ff 75 f4             	pushl  -0xc(%ebp)
     2f0:	e8 5d 0a 00 00       	call   d52 <dup>
      close(p[0]);
     2f5:	58                   	pop    %eax
     2f6:	ff 75 f0             	pushl  -0x10(%ebp)
     2f9:	e8 ac 0a 00 00       	call   daa <close>
      close(p[1]);
     2fe:	58                   	pop    %eax
     2ff:	ff 75 f4             	pushl  -0xc(%ebp)
     302:	e8 a3 0a 00 00       	call   daa <close>
      runcmd(pcmd->left);
     307:	58                   	pop    %eax
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     c5d:	e8 18 01 00 00       	call   d7a <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     c71:	50                   	push   %eax
     c72:	e8 cb 00 00 00       	call   d42 <fstat>
  close(fd);
     c77:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
     c7a:	89 c6                	mov    %eax,%esi
  close(fd);
     c7c:	e8 29 01 00 00       	call   daa <close>
  return r;
...
00000d42 <fstat>:
SYSCALL(fstat)
     d42:	b8 08 00 00 00       	mov    $0x8,%eax
     d47:	cd 40                	int    $0x40
     d49:	c3                   	ret    

00000d4a <chdir>:
SYSCALL(chdir)
     d4a:	b8 09 00 00 00       	mov    $0x9,%eax
     d4f:	cd 40                	int    $0x40
     d51:	c3                   	ret    

00000d52 <dup>:
SYSCALL(dup)
     d52:	b8 0a 00 00 00       	mov    $0xa,%eax
     d57:	cd 40                	int    $0x40
     d59:	c3                   	ret    

00000d5a <getpid>:
SYSCALL(getpid)
     d5a:	b8 0b 00 00 00       	mov    $0xb,%eax
     d5f:	cd 40                	int    $0x40
     d61:	c3                   	ret    

00000d62 <sbrk>:
SYSCALL(sbrk)
     d62:	b8 0c 00 00 00       	mov    $0xc,%eax
     d67:	cd 40                	int    $0x40
     d69:	c3                   	ret    

00000d6a <sleep>:
SYSCALL(sleep)
     d6a:	b8 0d 00 00 00       	mov    $0xd,%eax
     d6f:	cd 40                	int    $0x40
     d71:	c3                   	ret    

00000d72 <uptime>:
SYSCALL(uptime)
     d72:	b8 0e 00 00 00       	mov    $0xe,%eax
     d77:	cd 40                	int    $0x40
     d79:	c3                   	ret    

00000d7a <open>:
SYSCALL(open)
     d7a:	b8 0f 00 00 00       	mov    $0xf,%eax
     d7f:	cd 40                	int    $0x40
     d81:	c3                   	ret    
...
00000d9a <link>:
SYSCALL(link)
     d9a:	b8 13 00 00 00       	mov    $0x13,%eax
     d9f:	cd 40                	int    $0x40
     da1:	c3                   	ret    

00000da2 <mkdir>:
SYSCALL(mkdir)
     da2:	b8 14 00 00 00       	mov    $0x14,%eax
     da7:	cd 40                	int    $0x40
     da9:	c3                   	ret    
     ...
     de9:	85 d2                	test   %edx,%edx
{
  ...
  write(fd, &c, 1);
     ...
     e50:	e8 2d ff ff ff       	call   d82 <write>
...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
     ...
     ec0:	e8 bd fe ff ff       	call   d82 <write>
  ...
  write(fd, &c, 1);
     ...
     f21:	e8 5c fe ff ff       	call   d82 <write>
  ...
  write(fd, &c, 1);
     f3a:	e8 43 fe ff ff       	call   d82 <write>
  for(i = 0; fmt[i]; i++){
  ...
  write(fd, &c, 1);
     ...
     fd3:	57                   	push   %edi
     fd4:	53                   	push   %ebx
     fd5:	e8 a8 fd ff ff       	call   d82 <write>
        while(*s != 0){
  ...
  write(fd, &c, 1);
    ...
    1027:	e8 56 fd ff ff       	call   d82 <write>
        ap++;
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    113e:	56                   	push   %esi
    113f:	e8 1e fc ff ff       	call   d62 <sbrk>
  if(p == (char*)-1)
